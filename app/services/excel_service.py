...
from app.repositories.diploma_repository import insert_diploma_with_cursor
...
def build_excel_export(diplomas: list[dict], university_code: int):
    wb = Workbook()
    ws = wb.active
    ws.title = "Дипломы"
...
def bulk_import_diplomas(df, university_code: int):
    ...
    for idx in range(len(df)):
        ...
        try:
            ...
            if error.__class__.__name__ == "IntegrityError":
                error_count += 1
                errors.append(
                    f"Строка {idx + 1}: диплом с номером '{diploma_number}' уже существует"
                )
            else:
                error_count += 1
